...
public class BeekeeperTradeOffers {
    public static class EmeraldsForFlowers implements VillagerTrades.ItemListing {
        ...
        public MerchantOffer getOffer(Entity merchant, RandomSource random) {
            ...
            var tag = ItemTags.FLOWERS;
            var values = TagHelper.getValues(registryAccess.registryOrThrow(tag.registry()), tag);

            // Get all flowers without wither rose.
            var flowers = values.stream().filter(f -> !f.equals(Items.WITHER_ROSE)).toList();

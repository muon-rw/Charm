...
public class InventoryTidyingClient extends CharmFeature
    ...
    public void runWhenEnabled() {
        ...
        CharmApi.getProviderData(IHasScreenOffsetTweaks.class, provider -> provider.getScreenOffsetTweaks().stream())
            .forEach(t -> SCREEN_TWEAKS.put(t.getScreen(), t.getOffset()));

        BLOCK_ENTITY_SCREENS.addAll(CharmApi.getProviderData(IHasBlockEntityScreens.class,
            provider -> provider.getBlockEntityScreens().stream()));

        BLACKLISTED_SCREENS.addAll(CharmApi.getProviderData(IHasBlacklistedScreens.class,
    ...
    private void addSortingButton(Screen screen, int x, int y, Button.OnPress callback) {
        SORTING_BUTTONS.add(new ImageButton(x, y, 10, 10, 40, 0, 10, CoreResources.INVENTORY_BUTTONS, callback));
    }

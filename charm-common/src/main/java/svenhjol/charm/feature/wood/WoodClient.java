...
public class WoodClient extends CharmFeature {
    ...
    public List<BooleanSupplier> checks() {
        return List.of(() -> Charm.LOADER.isEnabled(Wood.class));
    }
            ...
            for (var modId : Wood.CREATIVE_TAB_ITEMS.keySet()) {
                ...
                for (String name : buildingBlocks) {
                    Optional.ofNullable(items.get(name)).ifPresent(
                        item -> CharmClient.REGISTRY.itemTab(item, CreativeModeTabs.BUILDING_BLOCKS, Items.ACACIA_BUTTON)
                    );
                }

                Optional.ofNullable(items.get("leaves")).ifPresent(
                    item -> CharmClient.REGISTRY.itemTab(item, CreativeModeTabs.NATURAL_BLOCKS, Items.ACACIA_LEAVES)
                );

                Optional.ofNullable(items.get("log")).ifPresent(
                    item -> CharmClient.REGISTRY.itemTab(item, CreativeModeTabs.NATURAL_BLOCKS, Items.ACACIA_LOG)
                );

                Optional.ofNullable(items.get("sapling")).ifPresent(
                    item -> CharmClient.REGISTRY.itemTab(item, CreativeModeTabs.NATURAL_BLOCKS, Items.ACACIA_SAPLING)
                );

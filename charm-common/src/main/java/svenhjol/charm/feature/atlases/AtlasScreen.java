...
public class AtlasScreen extends CharmContainerScreen<AtlasContainer> {
    ...
    private AtlasImageButton createButton(ButtonDirection dir) {
        return new AtlasImageButton(() -> getX() + LEFT + dir.left, () -> getY() + TOP + dir.top, dir.width, dir.height,
            dir.texStart, 0, dir.height, 2 * dir.height, CoreResources.INVENTORY_BUTTONS, it -> mapGui.buttonClick(dir));
    }
    ...
    protected void renderLabels(PoseStack poseStack, int mouseX, int mouseY) {
        ...
        poseStack.scale(BASE_SCALE, BASE_SCALE, 1);

        var bufferSource = Minecraft.getInstance().renderBuffers().bufferSource();
        var background = bufferSource.getBuffer(AtlasesClient.MAP_BACKGROUND);

        AtlasMapHelper.drawBackgroundVertex(poseStack, LIGHT, background);

        mapGui.render(poseStack, bufferSource, mouseX, mouseY);
    ...
    private void renderDecorations(PoseStack matrices, MultiBufferSource buffer, MapItemSavedData mapData, float relativeScale, Predicate<MapDecoration> filter) {
        ...
        for (MapDecoration mapdecoration : mapData.getDecorations()) {
            ...
            matrices.mulPose(Axis.ZP.rotationDegrees(mapdecoration.getRot() * 22.5f));

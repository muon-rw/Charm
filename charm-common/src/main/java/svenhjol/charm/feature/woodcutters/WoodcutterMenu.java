...
public class WoodcutterMenu extends AbstractContainerMenu {
    ...
    private void updateInput(Container input, ItemStack stack) {
        ...
        if (!stack.isEmpty()) {
            this.availableRecipes = this.level.getRecipeManager().getRecipesFor(Woodcutting.RECIPE_TYPE.get(), input, this.level);
        }
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            } else if (this.level.getRecipeManager().getRecipeFor(Woodcutting.RECIPE_TYPE.get(), new SimpleContainer(new ItemStack[]{itemStack2}), this.level).isPresent()) {
